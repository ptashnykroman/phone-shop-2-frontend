...
import { formatComparisonSpecificationLabel } from '@/features/comparison/lib/comparison-formatters'
...
export default async function ProductPage({ params }: ProductPageProps) {
  ...
  return (
    <div className="page-shell section-space space-y-8">
      <Breadcrumbs
        items={[{ label: 'Головна', href: '/' }, { label: 'Каталог', href: '/products' }, { label: product.name }]}
      />
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {product.specifications.slice(0, 6).map((specification) => (
                  <div key={specification.id} className="rounded-2xl border border-border/70 bg-muted/30 p-4">
                    <p className="text-sm text-muted-foreground">
                      {formatComparisonSpecificationLabel(specification.key, specification.label)}
                    </p>
      ...
      {explained.length > 0 ? <ExplainedSpecifications groups={explained} /> : null}
